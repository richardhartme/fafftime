...
describe('Core Logic Functions', () => {
  ...
  describe('mergeNearbySlowPeriods', () => {
    it('merges periods that are less than 1 minute apart', () => {
      const slowPeriods = [
        {
          startTime: new Date('2024-01-01T10:00:00Z'),
          ...
          startDistance: 100,
          endDistance: 110,
          gpsPoints: [[51.5074, -0.1278], [51.5075, -0.1279]]
        },
        {
          startTime: new Date('2024-01-01T10:05:30Z'), // 30 second gap
          endTime: new Date('2024-01-01T10:09:30Z'),
          recordCount: 4,
          startDistance: 110,
          endDistance: 120,
          gpsPoints: [[51.5076, -0.1280], [51.5077, -0.1281]]
        }
      ];

      const result = mergeNearbySlowPeriods(slowPeriods);

      expect(result).toHaveLength(1);
      expect(result[0].startTime).toEqual(new Date('2024-01-01T10:00:00Z'));
      expect(result[0].endTime).toEqual(new Date('2024-01-01T10:09:30Z'));
      expect(result[0].recordCount).toBe(9);
      expect(result[0].startDistance).toBe(100);
      expect(result[0].endDistance).toBe(120);
      expect(result[0].gpsPoints).toHaveLength(4);
    });

    it('does not merge periods that are 1+ minute apart', () => {
      const slowPeriods = [
        {
          startTime: new Date('2024-01-01T10:00:00Z'),
          endTime: new Date('2024-01-01T10:05:00Z'),
          ...
          startDistance: 100,
          endDistance: 110,
          gpsPoints: [[51.5074, -0.1278]]
        },
        {
          startTime: new Date('2024-01-01T10:06:00Z'), // 1 minute gap
          endTime: new Date('2024-01-01T10:11:00Z'),
          recordCount: 5,
          startDistance: 110,
          endDistance: 120,
          gpsPoints: [[51.5076, -0.1280]]
        }
      ];
      ...
      expect(result).toHaveLength(2);
      expect(result[0].startTime).toEqual(new Date('2024-01-01T10:00:00Z'));
      expect(result[1].startTime).toEqual(new Date('2024-01-01T10:06:00Z'));
    });

    it('handles empty array', () => {
      expect(mergeNearbySlowPeriods([])).toEqual([]);
    });
    ...
    it('handles single period', () => {
      ...
      const result = mergeNearbySlowPeriods(slowPeriods);
      expect(result).toEqual(slowPeriods);
    });
  });

  describe('mergeNearbyRecordingGaps', () => {
    const createGapPeriod = (startTime: Date, endTime: Date, startDistance: number, endDistance: number) => ({
      startTime,
      endTime,
      recordCount: 0,
      startDistance,
      endDistance,
      gpsPoints: [[51.5074, -0.1278]] as [number, number][],
      isGap: true,
      gapData: {
        startTime,
        endTime,
        gapDuration: endTime.getTime() - startTime.getTime(),
        gapDurationMinutes: Math.round((endTime.getTime() - startTime.getTime()) / (1000 * 60)),
        gapDurationHours: Math.round((endTime.getTime() - startTime.getTime()) / (1000 * 60)) / 60,
        startDistance,
        endDistance,
        startGpsPoint: [51.5074, -0.1278] as [number, number],
        endGpsPoint: [51.5075, -0.1279] as [number, number]
      }
    });

    it('merges gap periods that are less than 1 minute apart', () => {
      const gapPeriods = [
        createGapPeriod(
        ...
        createGapPeriod(
          new Date('2024-01-01T10:05:30Z'), // 30 second gap
          new Date('2024-01-01T10:10:00Z'),
          100, 100
        )
      ];

      const result = mergeNearbyRecordingGaps(gapPeriods);

      expect(result).toHaveLength(1);
      expect(result[0].startTime).toEqual(new Date('2024-01-01T10:00:00Z'));
      expect(result[0].endTime).toEqual(new Date('2024-01-01T10:10:00Z'));
      expect(result[0].isGap).toBe(true);
      expect(result[0].recordCount).toBe(0);
      expect(result[0].gpsPoints).toHaveLength(2);
    });

    it('does not merge gap periods that are 1+ minute apart', () => {
      const gapPeriods = [
        createGapPeriod(
          new Date('2024-01-01T10:00:00Z'),
          new Date('2024-01-01T10:05:00Z'),
        ...
        createGapPeriod(
          new Date('2024-01-01T10:06:00Z'), // 1 minute gap
          new Date('2024-01-01T10:11:00Z'),
          100, 100
        )
      ];
    ...
    it('handles empty array', () => {
      expect(mergeNearbyRecordingGaps([])).toEqual([]);
    });

    it('handles single gap period', () => {
      const gapPeriods = [createGapPeriod(
        new Date('2024-01-01T10:00:00Z'),
        new Date('2024-01-01T10:05:00Z'),
        100, 100
      )];
      ...
      const result = mergeNearbyRecordingGaps(gapPeriods);
      expect(result).toEqual(gapPeriods);
    });
  });
});
